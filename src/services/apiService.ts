...
export async function fetchFromAPI<T = unknown>(
  endpoint: string,
  options: SafeFetchRequestInit = {},
  fallback?: Fallback<T | ApiErrorResult>,
  mockData?: Fallback<T | ApiErrorResult>,
): Promise<T | ApiErrorResult> {
  const safeFallback: Fallback<T | ApiErrorResult> =
    fallback ?? (() => ({ error: true, message: `Request to ${endpoint} failed.` }));

  return safeFetch<T | ApiErrorResult>(
    endpoint,
    options,
    safeFallback,
    undefined,
    mockData ?? safeFallback,
  );
}
