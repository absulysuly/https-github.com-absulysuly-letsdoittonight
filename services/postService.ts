...
const runQuery = async (category?: 'general' | 'campus', limit = 20, offset = 0): Promise<Post[]> => {
  if (!supabase) {
    return fallbackPosts.filter((p) => !category || p.category === category)
  }
...
export const postService = {
  getGeneralPosts(limit = 20, offset = 0) {
    return runQuery('general', limit, offset)
  },
  getCampusPosts(limit = 20, offset = 0) {
    return runQuery('campus', limit, offset)
  },
  createPost: async (payload: Pick<Post, 'content' | 'category' | 'user_id'>): Promise<Post> => {
    if (!supabase) {
      return {
